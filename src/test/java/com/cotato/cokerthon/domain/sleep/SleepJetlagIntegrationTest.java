...
class SleepJetlagIntegrationTest {
	...
	void 목표보다_늦게_자는_경우_서쪽_방향_도시로_매칭된다() throws Exception {
		...
		assertThat(data.path("to").path("cityNameKr").asText()).isIn("뉴델리", "콜롬보");
		...
		// 지구본 매핑용 위경도가 함께 내려오는지 확인 (서울 → 매칭 도시)
		...
		assertThat(data.path("to").path("latitude").asDouble()).isNotZero();
		assertThat(data.path("to").path("longitude").asDouble()).isNotZero();
	}
	...
	void 같은_시차_구간에_여러_도시가_있으면_랜덤으로_매칭된다() throws Exception {
		...
		assertThat(matchedCities).containsExactlyInAnyOrder("베이징", "싱가포르", "타이베이");
	}
	...
	void 목표보다_일찍_자는_경우_동쪽_방향_도시로_매칭된다() throws Exception {
		...
		assertThat(data.path("to").path("cityNameKr").asText()).isIn("호니아라", "포트빌라", "누메아");
	}
